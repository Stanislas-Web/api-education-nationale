...
async function generateRapportActiviteJSON() {
  try {
    console.log('🔧 Génération du JSON rapport d\'activité pour le frontend...\n');
    ...
    // 2. Créer un rapport d'activité complet
    const rapportData = {
    ...
    // 4. Convertir en JSON pour le frontend
    const rapportJSON = rapport.toJSON();
    
    // 5. Ajouter les virtuals calculés
    rapportJSON.totalEffectifs = rapport.totalEffectifs;
    rapportJSON.totalInfrastructureBureaux = rapport.totalInfrastructureBureaux;

    // 6. Afficher le JSON formaté
    console.log('\n📄 JSON pour le frontend:');
    console.log(JSON.stringify(rapportJSON, null, 2));

    // 7. Sauvegarder dans un fichier
    const fs = require('fs');
    fs.writeFileSync('rapportActiviteExample.json', JSON.stringify(rapportJSON, null, 2));
    console.log('\n💾 JSON sauvegardé dans: rapportActiviteExample.json');

    console.log('\n🎉 Génération terminée! Vous pouvez utiliser ce JSON côté frontend.');
